...
/*
	NOTE: This cpu version is NOT implemented. Use gpu version instead.
*/
namespace caffe {

template <typename Dtype>
void AngleLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int num_output = this->layer_param_.angle_loss_param().num_output();  
  N_ = num_output;
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.angle_loss_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > center_filler(GetFiller<Dtype>(
        this->layer_param_.angle_loss_param().center_filler()));
...
template <typename Dtype>
void AngleLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  dotx.Reshape(M_, 1, 1, 1);
  dotb.Reshape(M_, 1, 1, 1);
  dotxb.Reshape(M_, 1, 1, 1);
  loss_data.Reshape(M_, 1, 1, 1);
}
...
REGISTER_LAYER_CLASS(AngleLoss);
